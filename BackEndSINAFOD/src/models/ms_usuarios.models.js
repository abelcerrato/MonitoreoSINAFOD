...
export const verificarUsuarioM = async (usuario) => {
  try {
    const { rows, rowCount } = await pool.query(
      "SELECT id, usuario, idrol, nombre, contraseña, correo, sesionactiva, cambiocontraseña, estado FROM ms_usuarios WHERE usuario = $1",
      [usuario]
    );
