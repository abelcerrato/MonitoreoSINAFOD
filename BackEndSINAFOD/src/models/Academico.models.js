...
export const getNivelAcademicoM = async (NivelAcademico) => {
    try {
        const { rows } = await pool.query('SELECT id FROM nivelesacademicos where nombre=$1 ', [NivelAcademico])
...
export const getCiclosAcademicosM = async () => {
    try {
        const { rows } = await pool.query('SELECT * FROM ciclosacademicos ORDER BY id asc')
        return rows;
    } catch (error) {
        throw error;
    }
...
export const getCicloAcademicoM = async (CicloAcademico) => {
    try {
        const { rows } = await pool.query('SELECT id FROM ciclosacademicos where nombre=$1', [CicloAcademico])
...
export const getCicloAcademicoIdNivelM = async (IdNivel) => {
    try {
        const { rows } = await pool.query(`SELECT c.nombre as ciclo, n.nombre as nivel FROM ciclosacademicos as c
                                            inner join nivelesacademicos n on c.idnivelesacademicos =n.id 
                                            where idnivelesacademicos=$1`, [IdNivel])
        return rows;
    } catch (error) {
        throw error;
    }
...
export const getGradoAcademicoM = async (GradoAcademico) => {
    try {
        const { rows } = await pool.query(`SELECT id FROM gradosacademicos where nombre=$1`, [GradoAcademico])
        return rows;
    } catch (error) {
        throw error;
    }
...
export const getGradoAcademicoIdCicloM = async (Ciclo) => {
    try {
        const { rows } = await pool.query(`
            ...
            order by ga.ciclo_educativo_id asc`, [Ciclo])
        return rows;
    } catch (error) {
        throw error;
    }
