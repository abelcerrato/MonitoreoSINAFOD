...
const CambiarContraModal = ({ open, onClose, mandatory = false, onSuccess }) => {
    ...
    return (
        <Modal open={open} onClose={mandatory ? undefined : onClose} disableEscapeKeyDown={mandatory}>
            <Box sx={style}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <Box sx={{ mt: 2, display: "flex", justifyContent: "flex-end" }}>
                        {!mandatory && (
                            <Button onClick={onClose} variant="outlined" sx={{ mr: 1 }} disabled={loading}>
                                Cancelar
                            </Button>
                        )}
                        <Button type="submit" variant="contained" disabled={loading} sx={{ background: color.primary.azul }} >
